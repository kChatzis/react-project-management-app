...
const KanbanBoard = () => {
 ...
 const onDragEnd = async result => {
  ...
  // For tasks
  const sourceColumn = board.find(column => column._id === source.droppableId);
  const task = sourceColumn.tasks.splice(source.index, 1)[0];

  const destinationColumn = board.find(column => column._id === destination.droppableId);
...
// change the task order on the backend 
  const updatedTasks = board.reduce((acc, column) => [...acc, ...column.tasks], []);
...
return (
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <Droppable droppableId="droppable" direction="horizontal" type="column">
        {(provided) => (
          <div className='kanban-card' ref={provided.innerRef} {...provided.droppableProps}>
            {board && board.map((column, index) => (
              <KanbanColumn key={column._id} column={column} index={index} />
            ))}
            {provided.placeholder}
            <span onClick={addColumn} className="material-symbols-outlined plus">add</span>          
            {modalIsOpen && <ColumnForm onCancel={closeModalHandler}></ColumnForm>}
          </div>
        )}
      </Droppable>
